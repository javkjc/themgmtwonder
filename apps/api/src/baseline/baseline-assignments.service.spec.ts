...
describe('BaselineAssignmentsService', () => {
    ...
    beforeEach(async () => {
        ...
        const module: TestingModule = await Test.createTestingModule({
            providers: [
                ...
                { provide: FieldAssignmentValidatorService, useValue: mockValidatorService },
                { provide: FieldLibraryService, useValue: mockFieldLibraryService },
                { provide: AuthorizationService, useValue: mockAuthService },
            ],
        }).compile();

        service = module.get<BaselineAssignmentsService>(BaselineAssignmentsService);
        dbService = module.get<DbService>(DbService);
        auditService = module.get<AuditService>(AuditService);
        validatorService = module.get<FieldAssignmentValidatorService>(FieldAssignmentValidatorService);
    ...
    describe('listAssignments', () => {
        it('should list assignments for a baseline', async () => {
            ...
            expect(authService.ensureUserOwnsBaseline).toHaveBeenCalledWith(mockUserId, mockBaselineId);
        });
        ...
        it('should include validation object when validationValid is not null', async () => {
            ...
            jest.spyOn(dbService.db, 'where').mockResolvedValueOnce([assignmentWithValidation]);
        ...
        it('should not include validation object when validationValid is null', async () => {
            ...
            jest.spyOn(dbService.db, 'where').mockResolvedValueOnce([assignmentWithoutValidation]);
    ...
    describe('upsertAssignment', () => {
        ...
        it('should create a new assignment successfully', async () => {
            ...
            jest.spyOn(dbService.db, 'returning').mockResolvedValueOnce([mockAssignment]);

            const result = await service.upsertAssignment(mockBaselineId, dto, mockUserId);
        ...
        it('should allow invalid value with explicit confirmation', async () => {
            ...
            const result = await service.upsertAssignment(mockBaselineId, dtoWithConfirm, mockUserId);
        ...
        it('should auto-normalize valid values with suggestions', async () => {
            ...
            const result = await service.upsertAssignment(mockBaselineId, dateDto, mockUserId);
        ...
        it('should require correctionReason when overwriting in reviewed status', async () => {
            ...
            jest.spyOn(authService, 'ensureUserOwnsBaseline').mockResolvedValue(reviewedContext);
        ...
        it('should allow overwriting in reviewed status with valid correctionReason', async () => {
            const reviewedContext = { ...mockBaselineContext, status: 'reviewed' };
            jest.spyOn(authService, 'ensureUserOwnsBaseline').mockResolvedValueOnce(reviewedContext);
            ...
            const result = await service.upsertAssignment(mockBaselineId, dtoWithReason, mockUserId);

            expect(result.assignment.correctedFrom).toBe('INV-001');
            expect(result.assignment.correctionReason).toBe('Updated invoice number per client request');
        });
        ...
        it('should throw BadRequestException if baseline is archived', async () => {
            ...
            jest.spyOn(authService, 'ensureUserOwnsBaseline').mockResolvedValue(archivedContext);
        ...
        it('should throw ForbiddenException if baseline is utilized', async () => {
            ...
            jest.spyOn(authService, 'ensureUserOwnsBaseline').mockResolvedValue(utilizedContext);
    ...
    describe('deleteAssignment', () => {
        it('should delete an assignment in draft status', async () => {
            ...
            const result = await service.deleteAssignment(mockBaselineId, mockFieldKey, mockUserId);
        ...
        it('should throw BadRequestException if correctionReason is too short', async () => {
            ...
            await expect(
                service.deleteAssignment(mockBaselineId, mockFieldKey, mockUserId, 'short')
            ).rejects.toThrow(BadRequestException);
        });
    });
    ...
    describe('getAggregatedBaseline', () => {
        ...
        const mockSegments = [
            { id: 'seg-1', text: 'Line 1', pageNumber: 1, confidence: null, boundingBox: null },
        ];
        ...
        it('should return null if no non-archived baseline exists', async () => {
            ...
            jest.spyOn(dbService.db, 'limit').mockResolvedValueOnce([archivedBaseline]);

            const result = await service.getAggregatedBaseline(mockAttachmentId, mockUserId);
