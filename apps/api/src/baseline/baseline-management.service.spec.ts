...
describe('BaselineManagementService', () => {
    ...
    const mockOcrResults = [
        { id: '1', attachmentOcrOutputId: mockOcrOutputId, fieldName: 'invoice_number', fieldValue: 'INV-001' },
        { id: '2', attachmentOcrOutputId: mockOcrOutputId, fieldName: 'invoice_date', fieldValue: '2024-01-15' },
    ];

    const mockFieldLibrary = [
        { id: '1', fieldKey: 'invoice_number', status: 'active', characterType: 'varchar' },
        { id: '2', fieldKey: 'invoice_date', status: 'active', characterType: 'date' },
    ];
    ...
    describe('createDraftBaseline', () => {
        it('should create a draft baseline successfully', async () => {
            jest.spyOn(dbService.db, 'returning').mockResolvedValueOnce([mockBaseline]);
            jest.spyOn(dbService.db, 'limit')
                .mockResolvedValueOnce([mockOcrOutput]) // Current OCR
                .mockResolvedValueOnce([]); // No existing segments

            const result = await service.createDraftBaseline(mockAttachmentId, mockUserId);
    ...
    describe('markReviewed', () => {
        it('should mark a draft baseline as reviewed', async () => {
            ...
            jest.spyOn(dbService.db, 'returning').mockResolvedValueOnce([reviewedBaseline]);
    ...
    describe('confirmBaseline', () => {
        ...
        it('should confirm a reviewed baseline successfully', async () => {
            ...
            jest.spyOn(dbService.db, 'transaction').mockImplementation((callback) => callback(txMock));
    ...
    describe('archiveBaseline', () => {
        ...
        it('should archive a confirmed baseline successfully', async () => {
            jest.spyOn(dbService.db, 'limit').mockResolvedValueOnce([confirmedBaseline]);
            jest.spyOn(dbService.db, 'returning').mockResolvedValueOnce([archivedBaseline]);

            const result = await service.archiveBaseline(mockBaselineId, mockUserId, 'Test reason');
    ...
    describe('markBaselineUtilized', () => {
        const confirmedBaseline = { ...mockBaseline, status: 'confirmed', utilizedAt: null };
        ...
        it('should throw NotFoundException if baseline not found', async () => {
            ...
            await expect(
                service.markBaselineUtilized(mockBaselineId, 'record_created', mockUserId)
            ).rejects.toThrow(NotFoundException);
        });

        it('should throw BadRequestException if status is not confirmed', async () => {
            const draftBaseline = { ...mockBaseline, status: 'draft', utilizedAt: null };
        ...
        it('should support all utilization types', async () => {
            const types: Array<'record_created' | 'process_committed' | 'data_exported'> = [
                'record_created',
                'process_committed',
                'data_exported',
            ];
            ...
            for (const type of types) {
                ...
                Object.assign(txMock, {
                    ...
                    update: jest.fn(() => txMock),
                    set: jest.fn(() => txMock),
                    returning: jest.fn().mockResolvedValueOnce([{ ...utilizedBaseline, utilizationType: type }]),
                });
