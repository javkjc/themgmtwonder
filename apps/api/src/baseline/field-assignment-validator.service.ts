...
export class FieldAssignmentValidatorService {
    ...
    private validateCurrency(value: string): ValidationResult {
        ...
        if (!iso4217Regex.test(trimmed)) {
            ...
            return {
                valid: false,
                error: 'Currency code must be exactly 3 uppercase letters (e.g., USD, EUR, GBP).'
            };
        }
    ...
    private validatePhone(value: string): ValidationResult {
        ...
        if (!/^\d+$/.test(cleaned)) {
            return {
                valid: false,
                error: 'Invalid phone number. Should contain only digits and optional formatting characters.',
            };
        }
    ...
    private validateBoolean(value: string): ValidationResult {
        ...
        return {
            valid: false,
            error: 'Invalid boolean value. Expected: true/false, yes/no, y/n, 1/0, on/off',
        };
    }
}
