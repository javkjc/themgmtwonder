...
describe('FieldAssignmentValidatorService', () => {
    ...
    beforeEach(async () => {
        ...
        service = module.get<FieldAssignmentValidatorService>(FieldAssignmentValidatorService);
    });

    describe('validate', () => {
        it('should allow null/empty values for all field types', () => {
            const types = ['varchar', 'int', 'decimal', 'date', 'currency', 'email', 'phone', 'url', 'percentage', 'boolean'];
    ...
    describe('validateVarchar', () => {
        it('should accept valid varchar values', () => {
            expect(service.validate('varchar', 'Test value')).toEqual({ valid: true });
            expect(service.validate('varchar', 'a'.repeat(100))).toEqual({ valid: true });
        });
    ...
    describe('validateInt', () => {
        ...
        it('should reject integers with commas', () => {
            ...
            expect(result.valid).toBe(false);
            expect(result.error).toContain('Thousands separators (commas) are not allowed');
    ...
    describe('validateEmail', () => {
        it('should accept valid email addresses', () => {
            expect(service.validate('email', 'test@example.com')).toEqual({ valid: true });
            expect(service.validate('email', 'user.name@domain.co.uk')).toEqual({ valid: true });
        });
    ...
    describe('validatePhone', () => {
        it('should accept valid phone numbers', () => {
            ...
            expect(service.validate('phone', '12345678901234')).toEqual({ valid: true });
        });
    ...
    describe('validateUrl', () => {
        it('should accept valid URLs', () => {
            ...
            const result3 = service.validate('url', 'https://example.com/path?query=1');
    ...
    describe('validateBoolean', () => {
        it('should accept various true representations', () => {
            const trueValues = ['true', 'True', 'TRUE', 'yes', 'YES', 'y', 'Y', '1', 'on', 'ON'];
        ...
        it('should accept various false representations', () => {
            const falseValues = ['false', 'False', 'FALSE', 'no', 'NO', 'n', 'N', '0', 'off', 'OFF'];
        ...
        it('should auto-normalize to true/false', () => {
            expect(service.validate('boolean', 'yes').suggestedCorrection).toBe('true');
            expect(service.validate('boolean', '1').suggestedCorrection).toBe('true');
            expect(service.validate('boolean', 'no').suggestedCorrection).toBe('false');
            expect(service.validate('boolean', '0').suggestedCorrection).toBe('false');
        });

        it('should not suggest correction when already normalized', () => {
            expect(service.validate('boolean', 'true').suggestedCorrection).toBeUndefined();
            expect(service.validate('boolean', 'false').suggestedCorrection).toBeUndefined();
        });
    ...
    describe('edge cases and integration', () => {
        it('should handle whitespace trimming across all validators', () => {
            ...
            expect(service.validate('int', '  123  ')).toEqual({ valid: true });
            expect(service.validate('email', '  test@example.com  ').valid).toBe(true);
        });

        it('should handle unicode and special characters in varchar', () => {
            expect(service.validate('varchar', 'Test 日本語 émojis 🎉')).toEqual({ valid: true });
        });

        it('should validate decimal precision correctly', () => {
            expect(service.validate('decimal', '123.456').suggestedCorrection).toBe('123.46');
            expect(service.validate('decimal', '123.001').suggestedCorrection).toBe('123.00');
        });
