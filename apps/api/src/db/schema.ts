...
export const todos = pgTable(
  ...
  (table) => ({
    ...
    schedulingIdx: index('todos_scheduling_idx').on(
      table.userId,
      table.startAt,
    ),
...
export const attachmentOcrOutputs = pgTable(
  ...
  (table) => ({
    attachmentIdIdx: index('attachment_ocr_outputs_attachment_id_idx').on(
      table.attachmentId,
    ),
...
export const ocrResults = pgTable(
  ...
  (table) => ({
    outputIdIdx: index('idx_ocr_results_output_id').on(
      table.attachmentOcrOutputId,
    ),
...
export const ocrJobs = pgTable(
  'ocr_jobs',
  {
    ...
    error: text('error'),
    outputId: uuid('output_id').references(() => attachmentOcrOutputs.id, {
      onDelete: 'set null',
    }),
  },
  (table) => ({
    userStatusIdx: index('idx_ocr_jobs_user_status').on(
      table.userId,
      table.status,
    ),
    statusRequestedIdx: index('idx_ocr_jobs_status_requested').on(
      table.status,
      table.requestedAt,
    ),
...
export const extractedTextSegments = pgTable(
  ...
  (table) => ({
    outputIdIdx: index('idx_extracted_text_segments_output_id').on(
      table.attachmentOcrOutputId,
    ),
  }),
);
...
export const ocrCorrections = pgTable(
  ...
  (table) => ({
    resultIdIdx: index('idx_ocr_corrections_ocr_result_id').on(
      table.ocrResultId,
    ),
    correctedByIdx: index('idx_ocr_corrections_corrected_by').on(
      table.correctedBy,
    ),
...
export const baselineFieldAssignments = pgTable(
  'baseline_field_assignments',
  {
    ...
    sourceSegmentId: uuid('source_segment_id').references(
      () => extractedTextSegments.id,
      { onDelete: 'set null' },
    ),
  ...
  (table) => ({
    baselineFieldUnique: unique('baseline_field_unique').on(
      table.baselineId,
      table.fieldKey,
    ),
    baselineIdx: index('idx_baseline_field_assignments_baseline_id').on(
      table.baselineId,
    ),
    fieldKeyIdx: index('idx_baseline_field_assignments_field_key').on(
      table.fieldKey,
    ),
    sourceSegmentIdx: index(
      'idx_baseline_field_assignments_source_segment_id',
    ).on(table.sourceSegmentId),
    modelVersionIdx: index(
      'idx_baseline_field_assignments_model_version_id',
    ).on(table.modelVersionId),
  }),
);
...
export const baselineTables = pgTable(
  ...
  (table) => ({
    baselineTableUnique: unique('baseline_table_unique').on(
      table.baselineId,
      table.tableIndex,
    ),
  }),
...
export const baselineTableCells = pgTable(
  ...
  (table) => ({
    cellUnique: unique('baseline_table_cell_unique').on(
      table.tableId,
      table.rowIndex,
      table.columnIndex,
    ),
    validationStatusIdx: index('idx_baseline_table_cells_validation').on(
      table.tableId,
...
export const baselineTableColumnMappings = pgTable(
  ...
  (table) => ({
    columnMappingUnique: unique('baseline_table_column_mapping_unique').on(
      table.tableId,
      table.columnIndex,
    ),
    tableIdx: index('idx_baseline_table_column_mappings_table_id').on(
      table.tableId,
    ),
  }),
);
...
export const mlModelVersions = pgTable(
  ...
  (table) => ({
    modelNameVersionUnique: unique(
      'ml_model_versions_model_name_version_unique',
    ).on(table.modelName, table.version),
...
export const mlTableSuggestions = pgTable(
  ...
  (table) => ({
    attachmentStatusIdx: index('idx_ml_table_suggestions_attachment_status').on(
      table.attachmentId,
      table.status,
    ),
  }),
...
export const attachmentOcrOutputsRelations = relations(
...
export const extractedTextSegmentsRelations = relations(
...
export const extractionBaselinesRelations = relations(
  extractionBaselines,
  ({ many }) => ({
...
export const baselineFieldAssignmentsRelations = relations(
...
export const baselineTablesRelations = relations(
  baselineTables,
  ({ one, many }) => ({
    baseline: one(extractionBaselines, {
...
export const baselineTableCellsRelations = relations(
  baselineTableCells,
  ({ one }) => ({
...
export const baselineTableColumnMappingsRelations = relations(
...
export const mlTableSuggestionsRelations = relations(
  mlTableSuggestions,
  ({ one }) => ({
    attachment: one(attachments, {
