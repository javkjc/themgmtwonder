...
export class TableSuggestionService {
    ...
    async detectTables(options: DetectTablesOptions) {
        const { attachmentId, userId, threshold = 0.6 } = options;
        ...
        return await this.dbs.db.transaction(async (tx) => {
            ...
            const [ocrOutput] = await tx
                ...
                .orderBy(desc(attachmentOcrOutputs.confirmedAt))
                .limit(1);

            if (!ocrOutput) {
                throw new BadRequestException('No confirmed OCR output found');
            }
            ...
            if (segments.length === 0) {
                throw new BadRequestException('No text segments available for detection');
            }
            ...
            // 5. Persist suggestions with status 'pending'
    ...
    async ignoreSuggestion(options: IgnoreSuggestionOptions) {
        ...
        return await this.dbs.db.transaction(async (tx) => {
            ...
            const [suggestion] = await tx
                ...
                .from(mlTableSuggestions)
                .leftJoin(attachments, eq(mlTableSuggestions.attachmentId, attachments.id))
                .leftJoin(todos, eq(attachments.todoId, todos.id))
