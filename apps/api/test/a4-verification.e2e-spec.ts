...
import { users, categories, todos, attachments, extractionBaselines, baselineFieldAssignments, auditLogs, fieldLibrary } from './../src/db/schema';
...
describe('A4 Verification (e2e)', () => {
    ...
    beforeAll(async () => {
        ...
        // 1. Register User
        const regRes = await agent.post('/auth/register').send({ email, password }).expect(201);

        // Extract CSRF token from Set-Cookie header
        const csrfCookieName = process.env.CSRF_COOKIE_NAME || 'todo_csrf';
        const setCookie = regRes.headers['set-cookie'] as string[];
        if (setCookie) {
            const csrfCookie = setCookie.find(c => c.startsWith(`${csrfCookieName}=`));
            if (csrfCookie) {
        ...
        // Get userId from DB
        const user = await dbService.db.query.users.findFirst({ where: eq(users.email, email) });
        userId = user!.id;

        // 2. Setup Data
        // Create Todo
        const [todo] = await dbService.db.insert(todos).values({
            userId, title: 'Test Todo', category: 'General'
        }).returning();
        todoId = todo.id;

        // Create Attachment
        const [att] = await dbService.db.insert(attachments).values({
            userId, todoId: todo.id, filename: 'test.pdf', storedFilename: 'test.pdf', mimeType: 'application/pdf', size: 1000
        }).returning();
        attachmentId = att.id;

        // Ensure Invoice Number field exists
        const field = await dbService.db.query.fieldLibrary.findFirst({ where: eq(fieldLibrary.fieldKey, 'invoice_number') });
        if (!field) {
    ...
    afterAll(async () => {
        // Cleanup resources
        if (baselineId) await dbService.db.delete(extractionBaselines).where(eq(extractionBaselines.id, baselineId));
        if (attachmentId) await dbService.db.delete(attachments).where(eq(attachments.id, attachmentId));
    ...
    it('Checkpoint A4: Full Flow', async () => {
        ...
        const draftRes = await agent.post(`/attachments/${attachmentId}/baseline/draft`)
        ...
        await agent.post(`/baselines/${baselineId}/assign`)
            .set('x-csrf-token', csrfToken)
            .send({
                fieldKey: 'invoice_number',
        ...
        const assign1 = await dbService.db.query.baselineFieldAssignments.findFirst({
            where: (t, { and, eq }) => and(eq(t.baselineId, baselineId), eq(t.fieldKey, 'invoice_number'))
        });
        ...
        const upsertLog = allLogs.find(l =>
        ...
        await agent.delete(`/baselines/${baselineId}/assign/invoice_number`)
            .set('x-csrf-token', csrfToken)
            .expect(400);
        ...
        const deleteLog = logsAfterDelete.find(l =>
        ...
        await dbService.db.update(extractionBaselines)
            .set({ status: 'archived' })
        ...
        const aggregatedRes = await agent.get(`/attachments/${attachmentId}/baseline`).expect(200);
        ...
        await dbService.db.update(extractionBaselines)
            .set({ status: 'draft', utilizedAt: new Date(), utilizationType: 'record_created' })
            .where(eq(extractionBaselines.id, baselineId));
