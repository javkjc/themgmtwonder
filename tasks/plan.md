...
**Problem statement**  
Users must explicitly accept, modify with reason, or clear suggestions with reason, and UI must reflect each state.
...
### E1 — Table Detection Trigger + Banner List ([Complexity: Medium])

**Problem statement**  
Users need a clear, explicit action to detect tables and see suggested banners without interrupting workflow.

**Files / Locations**
- Frontend: `apps/web/app/components/suggestions/TableSuggestionBanner.tsx` — new component.
- Frontend: `apps/web/app/attachments/[attachmentId]/review/page.tsx` — render banner list above FieldAssignmentPanel.
- Frontend: `apps/web/app/lib/api/tables.ts` — add table suggestion API helpers.
- Docs: `tasks/codemapcc.md` — update component map.

**Implementation plan**
1. Add “Detect Tables” button (explicit action) near the panel header.
2. On click, call `POST /attachments/:attachmentId/table-suggestions/detect`.
3. Render banner per suggestion with confidence badge and actions: Preview, Ignore.
4. Banner dismiss = Ignore action (persisted).

**Checkpoint E1 — Verification**
- Manual:
  - Click “Detect Tables” ? banners appear for suggestions.
...
- [ ] Detect tables
  - Steps: Click “Detect Tables” ? banner appears with row/column counts.
- [ ] Preview and convert
