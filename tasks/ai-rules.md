...
This file provides behavioral guidelines and workflow optimization for AI assistants working on this project. It complements (does not replace) the strict governance rules in `prompt_guidelines.md`.

**Authority Hierarchy:**
1. `prompt_guidelines.md` - Execution governance (MUST follow)
2. `ai-rules.md` - Behavioral standards (SHOULD follow)
3. When in conflict, prompt_guidelines.md always wins

## Required Reading on Session Start

Before beginning any work, read these files in order:

1. **prompt_guidelines.md** - Understand execution governance
...
- Use codemapcc.md for navigation instead of scanning repository
- Reference features.md for "why" context, not "what" to build (plan.md owns execution)

### 3. Self-Improvement Loop
- After ANY correction from user: update `tasks/lessons.md` with the pattern
- Write rules that prevent the same mistake
- Review lessons.md at session start for relevant patterns
...
- **Prevention**: Update lessons.md or add tests to catch similar issues

### 7. Context Preservation
- Before ending session: update session-state.md with current state
...
- Respect the "one concern per task" principle from prompt_guidelines.md

## Task Management Protocol

1. **Identify Task**: Find NEXT TASK from plan.md
2. **Verify Understanding**: Check requirements and files list
3. **Track Progress**: Mark items complete as you go (update plan.md status only)
...
- **Governance First**: prompt_guidelines.md rules are non-negotiable.
...
- **Respect Stop Conditions**: From prompt_guidelines.md - stop when task complete or clarity needed

## Anti-Patterns to Avoid

- ❌ Continuing when confused — STOP and ask (per prompt_guidelines.md)
...
- ❌ Violating any rule in prompt_guidelines.md

## File Update Rules

Per prompt_guidelines.md requirements:
...
Use this structure for all appends (per prompt_guidelines.md):
...
- **Both**:
  - Follow all prompt_guidelines.md governance rules
  - Maintain file update discipline
  - Review lessons.md at session start
  - Update session-state.md at session end

## Integration with prompt_guidelines.md

This file (ai-rules.md) provides **behavioral standards** that complement the **governance rules** in prompt_guidelines.md.

**Key differences:**
- prompt_guidelines.md: WHAT you must do (authority, scope, stop conditions)
- ai-rules.md: HOW you should do it (quality, style, learning, continuity)

**When in conflict:** prompt_guidelines.md always wins.

**Session structure:**
1. User crafts prompt following prompt_guidelines.md structure
2. User tells you to read ai-rules.md
